...
function PostCommonImages(props) {
    ...
    function fileType(elem) {
        ...
        if (type[1] === "mp4" || type[1] === "webm" || type[1] === "gif") {
            return <img src={'http://localhost:8000/files/pv_' + type[0] + '.jpg'} onClick={() => disp(mvSetLink(elem))} alt="" className="post-common-images__video" />
        }
        return <img src={'http://localhost:8000/files/pv_' + elem} onClick={() => disp(mvSetLink(elem))} alt="" className="post-common-images__img" />
    }
    return (
            ...
            {props.files && props.files.map((elem) => (
                <figure key={elem} className="post-common-images__img-wr">
                    <div className="post-common-images__title">
                        <a href={'http://localhost:8000/files/' + elem} target="_blank" className="post-common-images__name">{elem}</a>
                        <div className="post-common-images__title-btns">
                            <BtnSearch url={'http://localhost:8000/files/' + elem} />
                            <BtnDownload url={'http://localhost:8000/files/'} file={elem} />
                        </div>
                    </div>
                    {fileType(elem)}
                </figure>
